...
getArticle.style.display = 'flex'
getArticle.style.flexWrap = 'wrap';
getArticle.style.justifyContent = 'center';
getArticle.style.gap = '90px';
getArticle.style.width = '1000px'
...
getForm.forEach((e, i) => {
	...
	e.style.background= 'radial-gradient(red 10px, yellow 30%, #1e90ff 50%)';
	...
	e.addEventListener('submit', (element) => {
		...
		if (primer) {
			primer.push(info)
			localStorage.setItem('Pokemon', JSON.stringify(primer))
		}
		else {
